...
namespace BloomHarvester.Parse
{
	...
	public class ParseClient : Bloom.WebLibraryIntegration.BloomParseClient
	{
		...
		internal List<Book> GetBooks(out bool didExitPrematurely, string whereCondition = "", IEnumerable<string> fieldsToDereference = null)
		{
			...
			//request.AddParameter("keys", "object_id,baseUrl,harvestState,harvesterMajorVersion,harvesterMinorVersion,harvestLog,harvestStartedAt,title,inCirculation,langPointers,uploader,features");
			...
			results.ForEach(book => book.InitializeDatabaseVersion());
